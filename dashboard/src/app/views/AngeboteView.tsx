...
export function AngeboteView() {
  ...
  const [orderOffers, setOrderOffers] = useState<Record<number, Offer[]>>({});

  useEffect(() => {
    // Fetch offers for each order
    orders.forEach(async (order) => {
      if (!orderOffers[order.id]) {
        try {
  ...
  const mappedQuotes = useMemo(() => {
    return orders.map(order => ({
      id: order.id,
      customerName: order.contact?.name || 'Unbekannt',
      whatsappNumber: order.contact?.wa_id || '',
      oemNumber: order.oem,
      partName: order.part_json?.name || 'Kfz-Teil',
      ...
      options: (orderOffers[order.id] || []).map((off, idx) => ({
        ...
        supplier: off.supplierName || 'Lieferant',
        price: parseFloat(off.price.toString()),
